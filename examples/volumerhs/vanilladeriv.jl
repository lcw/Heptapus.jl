...
function main()
    ...
    CUDAdrv.@profile for _ = 1:ntrials
        @cuda(threads=(N+1, N+1, N+1), blocks=nelem,
              volumerhs_v2!(CUDA(), Val(3), Val(N), Val(nmoist), Val(ntrace), rhs,
    ...
    CUDAdrv.@profile for _ = 1:ntrials
        @cuda(threads=(N+1, N+1), blocks=nelem, maxregs=255,
              volumerhs_v3!(CUDA(), Val(3), Val(N), Val(nmoist), Val(ntrace), rhs,
                            Q, vgeo, DFloat(grav), D, nelem))
    end
    ...
    fill!(rhs, 0)
    @cuda(threads=(N+1, N+1), blocks=nelem, maxregs=255,
          volumerhs_v3_1!(CUDA(), Val(3), Val(N), Val(nmoist), Val(ntrace),
                          rhs, Q, vgeo, DFloat(grav), D, nelem))
    norm_v3_1 = norm(rhs)
    CUDAdrv.@profile for _ = 1:ntrials
        @cuda(threads=(N+1, N+1), blocks=nelem, maxregs=255,
